...
| EntryPoint | Specifies the application assembly that starts when the application is run. Must be an .exe file. If this is not a .Net Framework assembly, a bootstrapper must be used via the UseBootstrapper option. If not specified, the source directory is recursively searched for managed executables. If exactly one is found, it will be assigned to be the entry point. |
